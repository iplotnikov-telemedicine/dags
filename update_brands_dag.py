...
def get_ext_databases():
    ...
    query = '''
        ...
        WHERE schemaname like 'ext_indica_%_company'
    '''
    cursor.execute(query)
...
def upsert_brands(ti):
    customers, ext_databases = ti.xcom_pull(task_ids=['get_customers', 'get_ext_databases'])
    if not customers:
        ...
        for customer in customers:
            comp_id = customer[0]
            db_name = customer[1]
            if db_name not in ext_databases:
                logging.warning(f'External schema is not found for {db_name}')
            else:
...
with DAG(
    ...
    task_get_ext_databases = PythonOperator(
        task_id='get_ext_databases',
        python_callable=get_ext_databases,
        do_xcom_push=True
    )
