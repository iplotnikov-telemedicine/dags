...
def upsert_brands(schema, table, date_column, **kwargs):
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, id, brand_id, brand_name, wm_id, sync_created_at, sync_updated_at, description, is_internal, now() as inserted_at
...
def upsert_company_config(schema, table, **kwargs):
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            INSERT INTO {schema}.{table}
            SELECT {comp_id}, id, "name", human_name, value, now() as inserted_at
            FROM {ext_schema}.{table}
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, id, "name", "type", value, sync_created_at, sync_updated_at, 
                ...
                product_filter_id, created_at, updated_at, hide_banner, display_priority, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
...
def upsert_patient_group_ref(schema, table, date_column, **kwargs):
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            INSERT INTO {schema}.{table}
            SELECT {comp_id}, id, patient_id, group_id, sync_created_at, sync_updated_at, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
...
def upsert_patient_group(schema, table, date_column, **kwargs):
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            INSERT INTO {schema}.{table}
            SELECT {comp_id}, id, "name", sync_created_at, sync_updated_at, is_auto, start_date, end_date, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, pat_id, pat_user_id, pat_user_info, pat_office_id, pat_first_name, pat_middle_name, pat_last_name, 
                ...
                phone_consent_signature, email_consent_given_at, email_consent_signature, deleted_at, tax_tier_id, buyer_status, marketplace, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, id, "name", description, photo, lft, rgt, 
                ...
                label_template_internal_id, icon_color, system_id, now() as inserted_at
            FROM {ext_schema}.{table}
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
...
def upsert_product_filter_index(schema, table, **kwargs):
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            INSERT INTO {schema}.{table}
            SELECT {comp_id}, id, product_id, product_filter_id, now() as inserted_at
            FROM {ext_schema}.{table}
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT
                ...
                base_weight, product_checkin_to_id, office_name, now() as inserted_at
            FROM {ext_schema}.{table}
            WHERE {date_column} > (
                select coalesce(max({date_column}), '1970-01-01 00:00:00'::timestamp)
                from {schema}.{table}
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, id, name, description, phone, email, created_at, updated_at, patient_id, address, 
                ...
                metrc_license, vendor_type, registration_certificate, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, prod_id, prod_name, prod_price, prod_qty_w, prod_joints_qty_w, prod_qty_o, prod_joints_qty_o, 
                ...
                is_preroll, preroll_weight, product_type_id, prod_upc, is_tax_exempt, cast(is_under_package_control as boolean) as is_under_package_control, 
                directions, twcc, product_class, brand_id, potify_brand_product_id, wm_product_id, brand_product_strain_name, 
                prod_tax_tier_version_id, is_city_local_tax_exempt, brutto_weight, brutto_weight_validation, custom_cost, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id} as comp_id, id, register_id, opening_amount, cash_sales, drops, expected_drawer, actual_drawer, 
                ...
                dc_cash_change, vehicle_id, now() as inserted_at
            FROM {ext_schema}.{table}
            WHERE {date_column} > (
                select coalesce(max({date_column}), '1970-01-01 00:00:00'::timestamp)
                from {schema}.{table}
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, id, manager_sf_guard_user_id, name, status, change_status_at, "type", is_active, 
                ...
                dispatch_orders_based_on_delivery_zones, vehicle_id, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, id, order_id, order_item_id, product_id, state, county, city, 
                ...
                city_delivery_sales_tax, city_delivery_local_tax, excise_delivery_tax, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id} as comp_id, id, "number", patient_id, "type", status, order_status, payment_status, 
                ...
                delivery_estimated_time_of_arrival, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id} as comp_id, id, order_id, product_id, "name", descr, price_type, price_per, 
                ...
                is_exchanged, exchanged_at, product_brutto_weight, product_brutto_weight_validation, confirmed_at, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id} as comp_id, id, service_id, office_id, user_id, patient_id, doctor_id, 
                ...
                "free", payment_method, payment_amount, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, id, product_id, vendor_id, user_id, qty, price, "date", 
                ...
                packaged_by_id, manifest, is_sample_package, paused_to_datetime, excise_tax_paid, now() as inserted_at
            FROM {table}_{comp_id}_temp
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            ...
            SELECT {comp_id}, poq_id, poq_prod_id, poq_office_id, poq_qty, poq_item_type, 
                sync_created_at, sync_updated_at, reserved_qty, paused_qty, now() as inserted
            FROM {ext_schema}.{table}
        '''
        cursor.execute(query)
        logging.info(f'{cursor.rowcount} rows inserted for {comp_id} at {datetime.now()}')
...
def upsert_warehouse_order_logs(schema, table, date_column, **kwargs):
    ...
    for comp_id, ext_schema in customers:
        ...
        query = f'''
            INSERT INTO {schema}.{table}
            SELECT {comp_id} as comp_id, id, order_id, "type", sf_guard_user_id, order_courier_register_id, created_at, register_id, application, now() as inserted_at
            FROM {ext_schema}.{table}
            WHERE
                {date_column} > (
                    SELECT coalesce(max({date_column}), '1970-01-01 00:00:00'::timestamp)
